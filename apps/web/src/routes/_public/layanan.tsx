...
type PerusahaanTag = { id: string; perusahaanId: string; tag: string };
type LayananImage = { id: string; image: string };
type Layanan = { id: string; title: string; images: LayananImage[] };
...
function RouteComponent() {
	...
	return (
		<div>
			...
			<section className="relative min-h-screen overflow-hidden rounded-b-3xl bg-[#F1F2F6]">
				...
				<div className="relative z-10 text-center flex flex-col justify-center items-center min-h-screen px-6 sm:px-10 lg:px-20 max-w-5xl mx-auto pb-16">
					...
					<p className="mt-4 text-xs sm:text-lg text-white/80 ">
						Kami hadir sebagai mitra terpercaya untuk kebutuhan taman sintetis
						maupun fasilitas olahraga profesional Anda.
					</p>
				</div>
			...
			{/* ── Brand ── */}
			<section className="bg-[#F1F2F6] py-16 lg:py-32 px-6 sm:px-10 lg:px-20">
	...
	return (
		<>
			...
			<div className="flex flex-col gap-24 max-w-md">
				<div>
					<p className="text-sm tracking-widest text-[#518100] uppercase mb-3">
						{adinko?.title || "Adinko"}
					</p>
	...
	return (
		<>
			...
			<div className="grid lg:grid-cols-2 gap-8 mb-12">
				...
				<p className="max-w-lg text-[#313131] leading-relaxed lg:pt-10">
					Dari pemasangan rumput sintetis hingga pembangunan lapangan olahraga,
					kami menghadirkan layanan lengkap
				</p>
			</div>
...
function ImageCarousel({ images }: { images: LayananImage[] }) {
...
function LayananCard({ item }: { item: Layanan }) {
	return (
		<div className="rounded-2xl overflow-hidden bg-white border border-gray-100 shadow-sm">
			<ImageCarousel images={item.images} />
			<div className="p-4">
				<p className="text-sm font-semibold text-gray-900 pb-4">{item.title}</p>
			</div>
		</div>
	);
}
