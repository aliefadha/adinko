...
function TestimoniGrid() {
	const { kategoriList, testimoniList } = Route.useLoaderData();
	const [activeKategoriId, setActiveKategoriId] = useState<string | null>(null);
	const [displayLimit, setDisplayLimit] = useState(6);

	const filteredTestimoni =
		activeKategoriId === null
			? testimoniList
			: testimoniList.filter((t) => t.kategoriId === activeKategoriId);

	const displayedTestimoni = filteredTestimoni.slice(0, displayLimit);
	const hasMore = filteredTestimoni.length > displayLimit;
	...
	return (
		<>
			...
			{/* Review grid */}
			{displayedTestimoni.length > 0 ? (
				<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-10">
					{displayedTestimoni.map((item) => {
